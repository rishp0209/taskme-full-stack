      ...
      <Popover className="relative">
        <Popover.Button className="inline-flex items-center outline-none">
          <div className="w-8 h-8 flex items-center justify-center text-gray-800 relative">
            ...
            {data?.length > 0 && (
              <span className="absolute text-center top-0 right-1 text-[10px] text-white font-semibold w-3.5 h-3.5 rounded-full bg-red-600">
                {data?.length}
              </span>
            )}
          </div>
        ...
        >
          <Popover.Panel className="absolute -right-16 md:-right-2 z-10 mt-5 flex w-screen max-w-max  px-4">
            {({ close }) =>
              data?.length > 0 && (
                <div className="max-w-md shrink-0 flex-auto overflow-hidden rounded-md bg-white text-sm leading-6 shadow-lg ring-1 ring-gray-900/5">
                  <div className="">
                    {data?.slice(0, 5).map((item, index) => (
                      ...
                      >
                        <div className="mt-1 h-8 w-8 flex shrink-0 items-center justify-center rounded-lg bg-gray-200 group-hover:bg-white">
                          {ICONS[item.notiType]}
                        </div>
